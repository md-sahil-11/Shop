...
const fill = e => {
    ...
    if(inList.indexOf(itemid) !== -1) {
        showInfoMore('Item included already', 1400, 'yellow')
        mylist.style.display = 'none'
        return 
    }
    items[items.length -1].value = e.target.getAttribute('name')
    items[items.length -1].setAttribute('readonly', 'readonly')
    items[items.length -1].classList.add('read')   
    brands[items.length -1].value = e.target.getAttribute('brand')
    brands[items.length -1].setAttribute('readonly', 'readonly')
    brands[items.length -1].classList.add('read')   
    rates[rates.length -1].value = e.target.getAttribute('price')
    quantities[quantities.length - 1].value = 1
    quantities[quantities.length - 1].setAttribute('maxVal', e.target.getAttribute('quantity'))
    amts[amts.length - 1].value = e.target.getAttribute('price')
    amts[amts.length -1].setAttribute('itemprice', e.target.getAttribute('price'))
    rows[rows.length - 1].setAttribute('itemid', itemid)
    tot += parseFloat(amts[amts.length -1].value)
...
const fillCustomerDetail = e => {
    custName.value = e.target.getAttribute('name')
    custContact.value = e.target.getAttribute('contact')
}
...
ipcRenderer.on('inventoryItems:show', (e, data) => {
    for (let i of data) {
        ...
        span.setAttribute('itemid', i._id)
...
ipcRenderer.on('customerList:show', (e, data) => {
    for(let i of data) {
        ...
        span.addEventListener('click', fillCustomerDetail)
        li.appendChild(span)
...
const itemsFilter = e => {
    ...
    const li = document.querySelectorAll("#myMenu .li")
    const span = document.querySelectorAll("#myMenu .li .span")
    ...
    if(exist) showInfoMore("Inventory has no such item", 2000)
}
...
const removeItem = e => {
    ...
    if (rows.length > 1) {
        ...
        inList = inList.filter(val => val !== rows[index].getAttribute('itemid'))
        if(amts[index].value !== '')
            tot -= parseFloat(amts[index].value)
        rows[index].remove()
        listing(index)
    } else {
        inList = inList.filter(val => val !== rows[0].getAttribute('itemid'))
        items[0].value = ''
        items[0].removeAttribute('readonly')
        items[0].classList.remove('read')
        brands[0].value = ''
        brands[0].removeAttribute('readonly')
        brands[0].classList.remove('read')
        rates[0].value = ''
        quantities[0].value = 1
        tot -= parseFloat(amts[0].value)
        amts[0].value = '0'
    }
...
const sendData = data => {
    ipcRenderer.send('saleItems:add', data)
}
...
document.querySelector('form').addEventListener('submit', (e) => {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if(brands[i].value === '' 
        || items[i].value === ''
        || rates[i].value === '') continue
        data.push({
            'id': rows[i].getAttribute('itemid'), 
            'name': items[i].value, 
    ...
    sendData({
        ...
        'discount': parseFloat(dAmount.value),
...
const quantityChange = e => {
    ...
    const maxVal = parseFloat(e.target.getAttribute('maxVal'))
    if(parseFloat(e.target.value) > maxVal) {
        e.target.value = maxVal
        showInfoMore('Exceeding quantity limit!!!', 2000)
        amts[index].value = rate *maxVal
        tot = 0
        for(let i = 0; i < amts.length; i++) {
            
            tot += parseFloat(amts[i].value)
            console.log(tot)
        }
        subTotal.value = Number(tot.toFixed(2))
        discountVal()
    ...
    amts[index].value = isNaN(amt)? 0: amt
    tot += amt
    subTotal.value = isNaN(tot)? 0: Number(tot.toFixed(2))
    discountVal()
}
...
const rateChange = e => {
    ...
    if(items[items.length -1].getAttribute('readonly') !== 'readonly') return
    const index = e.target.getAttribute('index')
    const rate = parseFloat(rates[index].value)
    let tot = parseFloat(subTotal.value)
    let amt = parseFloat(amts[index].value)
...
custName.addEventListener('focus', e => {
    console.log('here')
    clist.style.display = 'block'
    clist.style.left = e.target.offsetLeft + 'px';
    clist.style.top = e.target.offsetTop + e.target.offsetHeight + 'px';
})
